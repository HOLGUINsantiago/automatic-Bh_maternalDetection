...
-   Data–sets most contain headers and be on csv format.

<!-- -->

-   Maternal cage column name must be "CageID", maternal cage pup id column name must be "PupID", adult and juvenile pup's IDs key columns name must be "id"

-   For data merging, maternal cage name is added to the actual id

-   actual cage for adult and ado df must be named "cage"

### Exporting df

```{r}
export_csv <- function(df, ..., sep = ";", row.names = FALSE, na = "") {
  # Recoger argumentos de ruta y file_name automáticamente
  path_parts <- list(...)
  if (length(path_parts) == 0) {
    stop("Debe proporcionar al menos el nombre del archivo.")
  }
  
  file_name <- path_parts[[length(path_parts)]]
  folders <- path_parts[-length(path_parts)]
  
  # Construir la ruta completa
  dir_path <- if (length(folders) > 0) here::here(folders) else here::here()
  ...
  # Guardar el CSV
  write.csv(df, full_path, sep = sep, row.names = row.names, na = na)
  
  message("CSV exportado a: ", full_path)
}


```

### Importing function and other declarations

```{r}
import_csv <- function(..., sep = ";", stringsAsFactors = TRUE, header = TRUE, encoding = "UTF-8", convert_empty_to_na = TRUE) {
  path <- here(...)  # Construye la ruta completa

  # Define los valores a tratar como NA solo si se indica
  na_vals <- if (convert_empty_to_na) c("", "NA") else NULL
...
cage_counts <- juvenile_df %>%
  group_by(CageID) %>%
  summarise(n_groups = n_distinct(Condition)) %>%
  filter(n_groups > 1)

# Para los CageID que se repiten en varios Condition, añadir la primera letra de Condition
juvenile_df <- juvenile_df %>%
  mutate(
    CageID = if_else(
      CageID %in% cage_counts$CageID,
      paste0(substr(Condition, 1, 1), "_", CageID),  # Añade la primera letra de Condition
      CageID
    )
  )

juvenile_df <- subset(juvenile_df, Project %in% c("VEAVE"))  # Keep only rows from the VEAVE project

ids_a_eliminar <- deadPups  
juvenile_df <- juvenile_df %>% 
  filter(!(PupID %in% ids_a_eliminar)) 

juvenile_df_only <- subset(juvenile_df, Cohort %in% c("Cohort 0", "Cohort 1", "Cohort 2"))  # Filter specific cohorts
...
```{r}
if(ado_data)
  ado_df <- import_csv("pups_data","ado","...")  
```
...
```{r}
if(adult_data)
  adult_df <- import_csv("pups_data","adult","...") 
```

### Mapping IDs

```{r}
if(adult_data || ado_data){
  mapps <- import_csv("pups_data", "...")

# Create a standardized Pup_label per individual, prefixed with "C" only if needed
mapps$Pup_label <- ifelse(
  startsWith(as.character(mapps$maternal_Cage), "C"),
  paste0(mapps$maternal_Cage, "-", mapps$pup_id),
  paste0("C", mapps$maternal_Cage, "-", mapps$pup_id)
)
}

if(ado_data)
  ado_df <- ado_df %>%
  left_join(mapps %>% select(cage, pup_id, Pup_label),
            by = c("cage", "pup_id"))
...
```{r}
days_to_keep <- c(1, 2, 3 ,4, 5, 6, 7 ,8)
#days_to_keep <- c(1, 2 ,4, 5, 6)

maternal_df <- import_csv("Resume_df.csv",sep =",")  

maternal_df$Transition <- maternal_df$Nest_exits_bouts + maternal_df$Nest_entry_bouts  # Calculate total transitions

maternal_df <- maternal_df %>%
  mutate(Day = floor(Time / 86400) + 1)  # Convert time (in seconds) to day number (1-indexed)

maternal_df <- maternal_df %>%
  filter(Day %in% days_to_keep) # Keep only days 1 to 7

if ("Start_time" %in% colnames(maternal_df)) {
  maternal_df <- maternal_df %>%
    mutate(Start_time = ymd_hms(gsub("T", " ", Start_time))) %>%  # Convert Start_time from ISO format to POSIXct
    filter(!(hour(Start_time) == 7 & minute(Start_time) >= 30 | hour(Start_time) > 7 & hour(Start_time) < 20))
  # Remove daytime (7:30 to 19:59)
}

maternal_full_df <- import_csv("behavioural_annotation.csv", sep=",")  # Load full maternal annotation data

maternal_full_df <- maternal_full_df %>% filter(Cage != "K2")

maternal_full_df <- maternal_full_df %>%
  mutate(Start_time = ymd_hms(gsub("T", " ", Start_time)))  # Convert Start_time to POSIXct format

maternal_full_df$Time <- NA_real_  # Initialize Time column

maternal_full_df <- maternal_full_df %>%
  group_by(Cage) %>%
  mutate(
    start_0 = min(Start_time, na.rm = TRUE),  # Define the starting timestamp per cage
    total_frames = max(frame, na.rm = TRUE),  # Get total number of frames per cage
    Time = as.numeric(difftime(Start_time, start_0, units = "secs")) +  # Compute elapsed time since start
      frame * Recording_duration / total_frames  # Adjust using frame index and recording duration
  ) %>%
  ungroup() %>%
  dplyr::select(-start_0, -total_frames) %>%
  mutate(Day = floor(Time / 86400) + 1)  # Compute the day number from total seconds

maternal_full_df <- maternal_full_df %>%
   filter(Day %in% days_to_keep) %>%
  filter(!(hour(Start_time) == 7 & minute(Start_time) >= 30 | hour(Start_time) > 7 & hour(Start_time) < 20))
 # Remove data from light phase (7:30–19:59)

maternal_df$Group <- as.factor(maternal_df$Group)  # Ensure Group is treated as a factor
...
```{r}
process_missing_data <- function(df, df_name = "df") {
  ...
  cat("\n[", df_name, "] Missing data percentages:\n")
  print(kable(missing_df, format = "markdown"))
  
  # Identify and remove columns with >30% missing data
  to_remove_missing <- names(missing_pct[missing_pct > 30])
  cat("\n[", df_name, "] Columns removed due to >30% missing:\n", to_remove_missing, "\n")
  df_clean <- df[, !(names(df) %in% to_remove_missing)]
  
  # Check if any columns remain to impute
  if (any(colMeans(is.na(df_clean)) * 100 > 0)) {
    cat("[", df_name, "] Applying missForest imputation...\n")
    
    # Identify factor columns with too many levels (>53)
    too_many_levels <- sapply(df_clean, function(col) is.factor(col) && nlevels(col) > 53)
    problematic_cols <- names(df_clean)[too_many_levels]
    
    # Separate problematic columns and data for imputation
    df_problematic <- df_clean[, problematic_cols, drop = FALSE]
    df_for_imputation <- df_clean[, !too_many_levels, drop = FALSE]
    
    df_for_imputation[] <- lapply(df_for_imputation, function(x) {
    ...
    # Recombine imputed and problematic columns, preserving original column order
    df_final <- df_clean[, c(names(df_imputed), problematic_cols)]
    df_final[, names(df_imputed)] <- df_imputed
    df_final[, problematic_cols] <- df_problematic
  } else {
    cat("[", df_name, "] No missing data to impute.\n")
    df_final <- df_clean
  }
...
# Example: apply on juvenile_df, ado_df, adult_df
juvenile_unic_completed <- process_missing_data(juvenile_df_only, "juvenile_df_only")

juvenile_all_completed <- process_missing_data(juvenile_df, "juvenile_df")

if(ado_data)
  ado_all_completed      <- process_missing_data(ado_df, "ado_df")

if(adult_data)
  adult_all_completed    <- process_missing_data(adult_df, "adult_df")
```
...
### CONT vs LBN day by day evolution box-plots
...
cluster_rf_analysis_mat <- function(data_df, 
  ...
  if(any(duplicated(df_cage_mean$CageID))){
    stop("CageID contiene valores duplicados. Por favor, revisa los datos.")
  }
...
export_csv(summary_cage, "maternal_analysis", "direct_condition_comparision", "on_nest_bouts.csv")
...
export_csv(summary_cage, "maternal_analysis", "direct_condition_comparision", "on_nest_duration.csv")
...
export_csv(summary_cage, "maternal_analysis", "direct_condition_comparision", "on-off_nest_ratio.csv")
...
export_csv(summary_cage, "maternal_analysis", "direct_condition_comparision", "build_duration.csv")
...
export_csv(summary_cage, "maternal_analysis", "direct_condition_comparision", "eat_drink_duration.csv")
...
export_csv(summary_cage, "maternal_analysis", "direct_condition_comparision", "eat_drink_bouts.csv")
...
export_csv(summary_cage, "maternal_analysis", "direct_condition_comparision", "on-off_nest_ratio_evolution.csv")
...
export_csv(summary_cage, "maternal_analysis", "direct_condition_comparision", "building_duration_evolution.csv")
```

### Transitions per day evolution (On nest bouts)
...
export_csv(summary_cage, "maternal_analysis", "direct_condition_comparision", "on_nest_bouts_evolution.csv")
```
...
export_csv(LBN_cage_means, "maternal_analysis", "LBN_behaviours_evolution", "change_point_and_evolution.csv")
...
## Clustering comportements maternels 

On force la formation de 3 groupes et on utilise seulement 3 dimensions de la PCA pour clustering
...
res_auto <- analyser_clusters_pca_mat(maternal_cleaned, colnames(maternal_cleaned),3, 3)
```
...
fit_VAR_models <- function(df, 
  ...
  # Temporal binning si se especifica
  if (!is.null(bin_size)) {
    df_proc <- df_proc %>%
  ...
  # Definir ID de video = Cage + Start_time + Group
  df_proc <- df_proc %>%
    mutate(Video_ID = paste0(Cage, "_", Start_time, "_", Group, "_", Day))
  
  # Selección de variables de comportamiento si no se especifica
  if (is.null(behav_vars)) {
    vars_to_exclude <- c(frame_col, "time_bin", "Cage", "Start_time", "LBN", 
                         ...
                         "Nest_exits", "Nest_entry", "Cage.ID", "Date", "Day", 
                         "Time", "cluster", "Condition", "Video_ID")
    behav_vars <- setdiff(colnames(df_proc), vars_to_exclude)
  }
  
  if (verbose) print(glue::glue("Variables de comportamiento utilizadas: {toString(behav_vars)}"))
  
  # Eliminar NA en variables de comportamiento
  df_proc <- df_proc %>%
    drop_na(all_of(behav_vars))
  
  # Consolidar duplicados por Video_ID + time_unit
  df_unique <- df_proc %>%
    group_by(Video_ID, .data[[time_unit]]) %>%
    summarise(across(all_of(behav_vars), max, na.rm = TRUE), .groups = "drop") %>%
    arrange(Video_ID, .data[[time_unit]])
  
  # Separar por Video_ID
  data_groups <- df_unique %>%
    group_split(Video_ID)
  
  names(data_groups) <- df_unique %>%
    pull(Video_ID) %>%
    unique() %>%
    as.character()
  ...
  var_models <- imap(data_groups, function(df_g, name) {
    ...
    if (auto_lag) {
      p <- tryCatch({
        ...
        warning(glue::glue("Lag auto fallido para {name}, usando p=1"))
        1
      })
    }
    ...
    tryCatch({
      ...
      warning(glue::glue("Error VAR para {name}: {e$message}"))
      NULL
    })
  })
...
```{r}
extract_P_matrix <- function(var_model) {
  coefs_list <- coef(var_model)
  variables <- names(coefs_list)  # Comportamientos como filas
  predictors <- rownames(coefs_list[[1]])
  predictors <- predictors[grepl(".l", predictors)]  # solo predictores con lags

  # Tus 9 comportamientos de interés
  beh_cols <- c("Onnest", "Offnest", "ABN", "Carryingpups", 
                "Selfgrooming", "Eat_drink", "Kicking", 
                "Groomingpups", "Build")

  n_vars <- length(variables)
  n_beh <- length(beh_cols)

  # Matriz P: filas = comportamientos observados, columnas = 9 comportamientos de interés
  P_matrix <- matrix(0, nrow = n_vars, ncol = n_beh)

  rownames(P_matrix) <- variables
  colnames(P_matrix) <- beh_cols

  for (i in seq_along(variables)) {
    for (j in seq_along(predictors)) {
      pred_name <- gsub(".l1", "", predictors[j])
      if (pred_name %in% beh_cols) {  # Solo si el predictor está en tus 9
        coef_value <- coefs_list[[i]][predictors[j], "Estimate"]
        P_matrix[variables[i], pred_name] <- coef_value
      }
    }
  }
...
```{r}
plot_VAR_graph <- function(coef_matrix,
                           var_name_labels = NULL,
                           min_width = 1,
                           max_width = 1.5,
                           edge_col = "#01cb01",
                           title = "Réseau comportemental VAR (probabilités de transition)") {
  # Matriz de coeficientes promedio
  coef_prom <- coef_matrix
  
  # Imprimir matriz cruda
  print(coef_prom)
  ...
  node_size <- min(9, max_label_length * 1 )
...
compare_VAR_models <- function(model_A, model_B, exclude = NULL, plot = TRUE, title = "Differences") {
...
## PCA et clustering (hclust)

### All variables affected by LBN (Strict filter) - true data 

We do not impose a specific number of clusters, the clusterated groups are automatic

```{r}
analyser_clusters_pca(juvenile_df, condition_impacted_strict, 2, -1)
```

First results validated = Good separation between CONT and LBN. We impose 8 observable groups

```{r}
analyser_clusters_pca(juvenile_df, condition_impacted_strict, 2, 8)
```

### All variables impacted by LBN condition (Not strict filter) - Completed data by Miss forest

We do not impose specific number of clusters, the clusterisation si automatic. A second clustering is done with random forest instead of PCA
...
\
There are two intrusive variables =\> Two LBN are into CONT cluster.

We impose the clustering to 6 because we could observe 6 groups on automatic clustering

```{r}
analyser_clusters_pca(juvenile_all_completed, condition_impacted_normal, 4, 6)
```

### All variables where there is a maternal effect - All data (missing data too)
...
### All variables where there is a maternal effect - Corrected data (missing forest)

```{r}
analyser_clusters_pca(juvenile_all_completed, cage_impacted_strict, 2, -1)
```

Impose 5 groups for clustering

```{r}
analyser_clusters_pca(juvenile_all_completed, cage_impacted_strict, 2, 5)
```

### All variables where there is a maternal effect (no strict filter) - All data (missing data too)
