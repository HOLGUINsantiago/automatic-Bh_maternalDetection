...
ids_a_eliminar <- deadPups  # List of PupIDs to remove : dead pups
juvenile_df <- juvenile_df %>% 
  filter(!(PupID %in% ids_a_eliminar))  # Filter out specific PupIDs
```
...
```{r}
# Load the pups mapping file (contains cage, pup_id, maternal_Cage, etc.)
...
# Join mapps to ado_df using keys: cage and pup_id
ado_df <- ado_df %>%
  left_join(mapps %>% select(cage, pup_id, Pup_label),
            by = c("cage", "pup_id"))

# Join mapps to adult_df using same keys
adult_df <- adult_df %>%
  left_join(mapps %>% select(cage, pup_id, Pup_label),
            by = c("cage", "pup_id"))
...
maternal_df <- import_csv("maternal_data","Resume_df.csv")  
...
maternal_full_df <- import_csv("maternal_data","behavioural_annotation.csv")  # Load full maternal annotation data
