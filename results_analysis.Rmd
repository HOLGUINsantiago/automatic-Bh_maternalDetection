...
maternal_df <- import_csv("maternal_data","Resume_df.csv",sep =",")  
...
maternal_df <- maternal_df %>%
  filter(Day >= 1 & Day <= 7)  # Keep only days 1 to 7
...
maternal_full_df <- import_csv("maternal_data","behavioural_annotation.csv", sep=",")  # Load full maternal annotation data
...
maternal_full_df <- maternal_full_df %>%
  filter(Day >= 1 & Day <= 7) %>%  # Keep days 1 to 7
  filter(!(hour(Start_time) == 7 & minute(Start_time) >= 30 | hour(Start_time) > 7 & hour(Start_time) < 20))
...
fit_VAR_models <- function(df, 
  ...
  if (!is.null(group_by_col)) {
    ...
    names(data_groups) <- df_unique %>% pull(group_by_col) %>% as.character() %>% unique()
  } else {
...
# Compare VAR models "A" and "1" excluding variables "Carryingpups" and "Kicking", generate plot with title "1 vs A"
...
# Extract unique Cage IDs and their experimental Groups, rename Group column to Condition
cage_conditions <- maternal_full_df %>%
  distinct(Cage.ID, Group) %>%
  rename(Condition = Group)

# Initialize a data frame to store comparison results between each cage VAR model and the control group VAR model "CONT"
diff_results <- map_dfr(names(cage_VARs), function(cage_id) {
  result <- tryCatch({
    # Compare VAR model of the current cage with the control group VAR model excluding "Carryingpups"
    diff_matrix <- compare_VAR_models(
    ...
    # Calculate the Frobenius norm of the difference matrix if it exists and is non-empty; otherwise assign NA
    ...
    # Return a data frame with Cage ID and the computed Frobenius distance
    data.frame(Cage.ID = cage_id, Frobenius_Dist = dist_val)
  }, error = function(e) {
    # If error occurs during comparison, return NA for that cage ID
    data.frame(Cage.ID = cage_id, Frobenius_Dist = NA)
  })
})

# Join the diff_results with cage_conditions to add experimental Condition information for each cage
...
```{r}
# 1. Get the list of Cage IDs from the cage_VARs list
cage_ids <- names(cage_VARs)

# 2. Initialize an empty square matrix with NA, with row and column names as cage IDs
n <- length(cage_ids)
frobenius_mat <- matrix(NA, nrow = n, ncol = n, dimnames = list(cage_ids, cage_ids))

# 3. Fill the matrix by computing pairwise Frobenius distances between VAR models
for (i in seq_along(cage_ids)) {
  for (j in seq_along(cage_ids)) {
    if (i <= j) {  # Compute only upper triangle and diagonal (matrix is symmetric)
      mat_i <- cage_VARs[[cage_ids[i]]]
      mat_j <- cage_VARs[[cage_ids[j]]]
      
      # Calculate Frobenius distance of difference matrix, handling errors gracefully
      dist_val <- tryCatch({
        diff_matrix <- compare_VAR_models(
          model_A = mat_i,
          model_B = mat_j,
          exclude = c("Carryingpups"),  # Exclude this variable from comparison
          plot = FALSE
        )
        
        if (!is.null(diff_matrix) && nrow(diff_matrix) > 0 && ncol(diff_matrix) > 0) {
          norm(as.matrix(diff_matrix), type = "F")  # Frobenius norm of difference matrix
        } else {
          NA  # Return NA if difference matrix is null or empty
        }
      }, error = function(e) NA)
      
      # Assign the computed distance to both symmetric positions in the matrix
...
# 4. Convert the matrix to a tidy data frame in long format
frobenius_df <- as.data.frame(as.table(frobenius_mat)) %>%
  rename(CageA = Var1, CageB = Var2, Frobenius_Dist = Freq) %>%
  filter(CageA != CageB) %>%  # Remove self-comparisons
  drop_na()  # Remove rows with NA distances

frobenius_mat <- frobenius_mat[, colSums(!is.na(frobenius_mat)) > 0]

# Eliminar filas que son completamente NA
frobenius_mat <- frobenius_mat[rowSums(!is.na(frobenius_mat)) > 0, ]

# 5. Plot heatmap of Frobenius distances between VAR models
pheatmap(
  frobenius_mat, 
  main = "Frobenius Distance Between VAR Models",
  display_numbers = TRUE,      # Display numerical values on the heatmap
